...
const {
  ...
  controlsEmbed,
  whitelist_Embed,
  CancelPreDelete_Embed,
  predelete_Embed,
  ownerLeave_Embed,
} = require("./embeds.js");
console.log(`Config Loaded prefix: ${process.env.prefix}`);
var owners = process.env.owners;
...
client.on("voiceStateUpdate", async (oldMember, newMember) => {
  ...
  let tempTimestamp = await db
    .get(`tempTimestampKey_${userid}`)
    .catch((err) => {}); //= await kvs1.get('tempTimestampKey_'+userid);

  if (newUserChannel == process.env.HUBvcChannelID) {
  ...
  if (!newUserChannel || newUserChannel != tempVcId) {
    if (typeof tempVcId !== "undefined") {
      ...
      let Vc = await guild.channels.cache.find(
        (channel) => channel.id == tempVcId,
      );
      if (!Vc) {
        await db.del(`tempVcIdKey_${userid}`).catch((err) => {
          console.log("Error kv!");
        });
        await db.del(`tempmsgIdKey_${userid}`).catch((err) => {
          console.log("Error kv!");
        });
        await db.del(`tempTimestampKey_${userid}`).catch((err) => {
          console.log("Error kv!");
        });
        return;
      }
      let msg = await Vc.send({
        content: `<@${newMember.id}>`,
        embeds: [ownerLeave_Embed],
      });
      let tout = setTimeout(RemoveVTChannels, 1000, userid, tempVcId);
...
async function MoveUser(userid, Member) {
  ...
  await Member.setChannel(newVCid).catch((err) => {
    /*remove channel*/
  });
}
...
async function RemoveVTChannels(UserID, VCID) {
  ...
  if (typeof VC !== "undefined")
    VC.delete().catch(() => {
      console.log("VC delete error");
    });
  await db.del(`tempVcIdKey_${UserID}`).catch((err) => {
    console.log("kv del error!");
  });
  await db.del(`tempmsgIdKey_${UserID}`).catch((err) => {
    console.log("kv del error!");
  });
...
async function CreatVTChannels(UserID) {
  ...
  const newTempVoiceChannel = await guild.channels.create({
    ...
    parent: process.env.categoryID,
    permissionOverwrites: [
      {
        id: process.env.DefaultRoleID,
  ...
  await db.put(`tempVcIdKey_${UserID}`, newTempVoiceChannel.id).catch((err) => {
    console.log("kv save error!");
  });
  await db.put(`tempmsgIdKey_${UserID}`, msg.id).catch((err) => {
    console.log("kv save error!");
  });
  await db.put(`tempTimestampKey_${UserID}`, Date.now() / 1000).catch((err) => {
    console.log("kv save error!");
  });
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isRoleSelectMenu() || interaction.isUserSelectMenu()) {
    if (
      interaction.customId == "menu_whitelist_member" ||
      interaction.customId == "menu_whitelist_role"
    ) {
      ...
      await interaction.update({
        ...
        components: [],
        content: "",
        embeds: [whitelist_Embed],
      });

      if (interaction.customId == "menu_whitelist_role") {
        let wrole = guild.roles.cache.find(
        ...
        let wUserid = client.users.cache.find(
          (user) => user.id == interaction.values[0],
        );
        let VCPermissions = await VC.permissionsFor(wUserid.id).serialize();
        //console.log(typeof(VCPermissions.VIEW_CHANNEL))
        //console.log(VCPermissions)
        if (!VCPermissions.ViewChannel) {
          await VC.permissionOverwrites.create(wUserid.id, {
            ViewChannel: true,
          });
        } else {
      ...
      whitelist_Embed.setFields([
        // {
        // name: "Roles",
        // value: `${str1_role != "" ? str1_role : "none"}`,
        // inline: true,
        // },
        {
          name: "Beyazliste Üyeler",
          value: `${str1_member != "" ? str1_member : "none"}`,
          inline: true,
        },
      ]);
    ...
    } else if (interaction.customId == "menu_banlist_member") {
      await interaction.update({
        ephemeral: true,
        components: [],
        content: "",
      ...
      if (interaction.customId == "menu_banlist_role") {
        let banrole = guild.roles.cache.find(
          (role) => role.id == interaction.values[0],
        );
        let VCPermissions = VC.permissionOverwrites.cache.find(
          (value) => value.id == banrole.id,
        );

        if (
          typeof VCPermissions === "undefined" ||
          !VCPermissions.deny.serialize().ViewChannel
        ) {
          await VC.permissionOverwrites.create(banrole.id, {
            ViewChannel: false,
          });
        } else {
        ...
        let banUserid = guild.members.cache.find(
          (user) => user.id == interaction.values[0],
        );
        ...
        let VCPermissions = VC.permissionOverwrites.cache.find(
          (value) => value.id == banUserid.id,
        );
        if (
          typeof VCPermissions === "undefined" ||
          !VCPermissions.deny.serialize().ViewChannel
        ) {
          await VC.permissionOverwrites.create(banUserid.id, {
            ViewChannel: false,
          });
        } else {
    ...
    if (interaction.customId === "select_kick") {
      ...
      let VCPermissions = await VC.permissionsFor(
        process.env.DefaultRoleID,
      ).serialize();
      Controlmsg.edit(
        await CreateControlMsg(
          newUser.id,
      ...
      if (removeTimeouts[user.id]) {
        ...
        delete removeTimeouts[userid];
        let Controlmsg = await VC.messages.fetch(
          `${removeTimeoutsMsg[userid]}`,
        );
        await Controlmsg.delete().catch(() => {});
      }
      ...
      if (removeTimeouts2[user.id])
        return await interaction.update({
          content: `No. Vc is delete now.`,
          components: [],
        });

      await db.del(`tempVcIdKey_${user.id}`).catch((err) => {
        console.log("kv del error!");
      });
      await db.del(`tempmsgIdKey_${user.id}`).catch((err) => {
        console.log("kv del error!");
      });
      await db.del(`tempTimestampKey_${user.id}`).catch((err) => {
        console.log("kv del error!");
      });

      await db.put(`tempVcIdKey_${newUser.id}`, tempVcId).catch((err) => {
        console.log("kv save error!");
      });
      await db.put(`tempmsgIdKey_${newUser.id}`, tempmsgid).catch((err) => {
        console.log("kv save error!");
      });
      await db
        .put(`tempTimestampKey_${newUser.id}`, tempTimestamp)
        .catch((err) => {
          console.log("kv save error!");
        });

      await interaction.update({
        content: `OK. GoodBye ${user}.`,
        components: [],
      });

      await VC.setName(
        `${newUser.nickname ? newUser.nickname : newUser.user.username}`,
      ).catch((err) => {
        console.log("kv save error!");
      });
      return;
    } else if (interaction.customId === "select_Region") {
      await VC.setRTCRegion(
        interaction.values[0] == "null" ? null : interaction.values[0],
      );

      return await interaction.update({
        content: `Odanın bölgesi ${VC.rtcRegion ? VC.rtcRegion : "Auto"} olarak güncellendi.`,
        components: [],
      });
    }
    ...
    if (interaction.customId === "modal_limit") {
      ...
      if (
        ...
        return interaction
          .reply({
            content:
              "Lütfen düğmeye basmadan önce sahibi tarafından doğru değeri girin.\nLütfen düğmeye basmadan önce doğru miktarı girin。",
            ephemeral: true,
          })
          .catch((err) => {
            VC.send("Unknow Error. Pls try again.");
          });

      await VC.setUserLimit(Number(inputnum));
      return interaction
        .reply({
          content: `Maksimum ses sayısı şu şekilde ayarlandı: **${Number(inputnum)}**`,
          ephemeral: true,
        })
        .catch((err) => {
          VC.send(
      ...
      return interaction
        .editReply({
          content: `Odanın ismi şu şekilde ayarlandı: **${inputstr}**`,
          ephemeral: true,
        })
        .catch((err) => {
          VC.send(
    ...
    if (tempVcId != textchannel.id) {
      return await interaction
        .reply({ content: "Uhhhhh... This is not your VC.", ephemeral: true })
        .catch((err) => {
          console.log(err);
        });
    }
    ...
    if (interaction.customId === "button_lock") {
      ...
      await VC.permissionOverwrites.edit(process.env.DefaultRoleID, {
        Connect: VcPermissions.Connect,
      });

      await Controlmsg.edit(
        await CreateControlMsg(user.id, user.username, VcPermissions),
      );
      await interaction
        .editReply({
          content: `Kanalın izinleri **${!VcPermissions.Connect ? "kilitli**" : "kilidi açık**"} olarak güncellendi.`,
          // content: `Kanalın durumu **${!VcPermissions.ViewChannel ? "gizli**" : "herkese açık**"} olarak değiştirildi.`,
          ephemeral: true,
        })
        .catch((err) => {
          VC.send(
      ...
      await VC.permissionOverwrites.edit(process.env.DefaultRoleID, {
        ViewChannel: VcPermissions.ViewChannel,
      });
      ...
      await interaction
        .editReply({
          ...
          content: `Kanalın durumu **${!VcPermissions.ViewChannel ? "gizli**" : "herkese açık**"} olarak güncellendi.`,
          ephemeral: true,
        })
        .catch((err) => {
          VC.send(
      ...
      await interaction
        .editReply({
          content: `Odaya yeni katılan kullanıcılar **${VcPermissions.Speak ? "konuşabilir**" : "konuşamaz**"} olarak güncellendi.`,
          ephemeral: true,
        })
        .catch((err) => {
          VC.send(
      ...
      if (kickUser.size <= 1)
        return interaction
          .editReply({ content: `Kullanıcı bulunamadı.`, ephemeral: true })
          .catch((err) => {
            VC.send(
              `Something is wrong. Please try again.\nError catch: \n\`\`\` ${err}\n\`\`\``,
            );
      ...
      return await interaction.editReply({
        content: "Lütfen istediğiniz bölgeyi seçin.",
        components: [row],
        ephemeral: true,
      });
      ...
      return await interaction.editReply({
        content: "Please select the one you want.",
        components: [row],
        ephemeral: true,
      });
      ...
      if (removeTimeouts2[user.id]) {
        ...
        let Controlmsg = await Vc.messages.fetch(
          `${removeTimeoutsMsg2[user.id]}`,
        );
        clearTimeout(removeTimeouts2[user.id]);
        delete removeTimeouts2[user.id];
        await Controlmsg.delete().catch((err) => {
          console.log(err);
        });
      } else {
        predelete_Embed.setDescription(
          `Odanın sahibi <#${tempVcId}> isimli odayı silmeye karar verdi. \nKanal <t:${Math.floor(Date.now() / 1000) + 10}:R> silinecektir. \nIptal etmek için kanalı sil butonuna tekrar basın.`,
        );
    ...
    if (interaction.commandName == "vc-fix") {
      ...
      if (!process.env.owners.includes(user.id)) return;
      for await (const key of db.keys()) {
        if (key.startsWith("tempVcIdKey_")) {
          ...
          adminDeleteEmbed.setDescription(
      ...
      return await interaction.editReply({
        content: "OK. Self-detonation start.",
        ephemeral: true,
      });
...
async function CreateControlMsg(
  UserID,
  userName,
  Permissinos = { ViewChannel: false, Connect: true, Speak: true },
) {
  let button1 = new ButtonBuilder()
    .setStyle(ButtonStyle.Primary)
    .setEmoji(`${Permissinos.Connect ? "🔒" : "🔓"}`)
    .setLabel(`${Permissinos.Connect ? "Kanalı Kilitle" : "Kilidi Kaldır"}`)
    .setCustomId("button_lock");
  let button2 = new ButtonBuilder()
    .setStyle(ButtonStyle.Primary)
    .setEmoji(`${Permissinos.ViewChannel ? "👤" : "👥"}`)
    .setLabel(`${Permissinos.ViewChannel ? "Kanalı Gizle" : "Kanalı Göster"}`)
  ...
  let buttonRow3 = new ActionRowBuilder().addComponents([
    button8,
    button5,
    button14,
  ]);
...
client
  ...
  .catch(() =>
    console.log("Invalid Token.Make Sure To Fill config.js or set ENV"),
  );
